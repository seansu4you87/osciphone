...
const char * parseFilePath( const char * filePath )
{
	
	//TODO: actually parse the file path
	...
	if(isSubpath)
		return [[[NSBundle mainBundle] pathForResource:name ofType:extension inDirectory:subpath ] UTF8String];
	else
		return [[[NSBundle mainBundle] pathForResource:name ofType:extension] UTF8String];
	
	
	
}

bool MoAudioFileIn::openFile( const char * fileName, bool raw, bool doNormalize, bool generate )
{	
    unsigned long lastChannels = channels;
    unsigned long samples, lastSamples =( bufferSize+1 )*channels;
	
	//Parse the fileName
    m_filename = parseFilePath(fileName);
    ...
    if( !generate || !strstr( fileName, "special:" ) )
    {
        ...
        if( !fd ) {
            fprintf( stderr, "[mopho](via MoAudioFileIn): could not open or find file( %s ).", fileName );
        ...
        if( fileSize == 0 ) {
            fprintf( stderr, "[mopho](via MoAudioFileIn): file( %s ) data size is zero!", fileName );
            // handleError( msg, StkError::FILE_ERROR );
            return false;
        }
    }
